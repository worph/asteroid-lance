...
export class BulletFactory {
    ...
    create(position: { x: number, y: number }, rotationRad: number, parentVelocity: { x: number, y: number }): EntityInterface {
        ...
        let eid = this.assetIDGenerator.generateAssetID(AssetIDGenerator.BULLET_PREFIX);
        ...
        let positionUpdate = {
            x: direction.x * ShipFactory.CONST.SIZE_RADIUS,
            y: direction.y * ShipFactory.CONST.SIZE_RADIUS
